...
1. **API path** — Provide the path for the operation you need. Examples:
   - Compile a template: `POST /v1/templates/compile` (template id is in the JSON body, not the path).
   - Send a message: `POST /v1/templates/send` (see [API reference](/api-reference/introduction) for body fields such as `channel_id`).
   - List templates: `GET /v1/templates`.
2. **Query params** — Add any query parameters the endpoint requires (e.g. `include_sample_data=true` for list templates). Set them in the query configuration when you invoke the resource.
3. **Body** — For compile or send, the body is JSON with `template_id`, `data` (same shape as [sample data](/guides/templates-sample-data)), and for send a destination as documented. Use Retool bindings to map form fields, table data, or state into `data`.
